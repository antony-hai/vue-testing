    ...
    <Sider hide-trigger :style="{background: '#fff'}">
        <Menu :active-name="activeName" theme="light" width="auto" :open-names="[openName]">
            <Submenu v-for="(item, index) in secondMenu" :name="item.name" :key="index">
                ...
                <MenuItem v-for="(menu, index) in item.children" :key="index" :to="menu.url" :name="menu.name">
                  {{ menu.title }}
                </MenuItem>
            </Submenu>
        </Menu>
...
export default {
  ...
  created() {
    ...
    const thirdMenuFirst = secondMenuFirst.children ? secondMenuFirst.children[0] : {}
